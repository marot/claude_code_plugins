 ...
 */
export function buildParentMap(node, parentMap = new Map(), parent) {
    if (parent) {
 ...
 */
export function searchHierarchy(node, pattern, searchIn, parentMap, path = []) {
    const results = [];
 ...
 */
export function findParent(node, levels, parentMap) {
    let current = node;
 ...
 */
function isContainedIn(nodeA, nodeB) {
    if (nodeA === nodeB) {
        return true;
    }
 ...
 */
export function deduplicateSubtrees(subtrees) {
    ...
    for (const candidate of subtrees) {
        ...
        if (shouldAdd) {
            ...
            const filtered = deduplicated.filter(existing => !isContainedIn(existing, candidate));
 ...
 */
export async function getHierarchy(options = {}) {
    ...
    if (options.query) {
        const { query, searchIn = ['text', 'resource-id', 'content-desc'], parentLevels = 0, } = options;
        ...
        try {
            ...
            throw new Error(`Invalid regex pattern: ${query}. Error: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        if (matches.length === 0) {
            console.log(JSON.stringify({ message: 'No matches found', query, searchIn }, null, options.json ? 2 : 0));
            return;
        }
        // Find parent nodes at specified level
        const parentNodes = matches.map((match) => findParent(match.node, parentLevels, parentMap));
        // Extract subtrees
